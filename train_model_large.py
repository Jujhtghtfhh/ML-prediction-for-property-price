...
# ============================================================
# KONFIGŪRACIJA - PAKEISKITE PAGAL SAVO POREIKIUS
# ============================================================

# Kelias į jūsų duomenis (pakeiskite į savo kelią)
DATA_DIR = "./data"  # <- PAKEISKITE ČIA

# Arba naudokite santykinį kelią
# DATA_DIR = "./Real Estate Southern Spain 2024"

# Modelių išsaugojimo vieta
MODEL_DIR = "./models"

# Apmokymo parametrai
TEST_SIZE = 0.2           # 20% duomenų testavimui
RANDOM_STATE = 42         # Pakartojamumui
MAX_IMAGES_PER_PROPERTY = 3  # Maks. nuotraukų skaičius (greičiui)
NUM_WORKERS = 4           # Lygiagretaus apdorojimo gijos
...
def parse_numeric(val):
    """Parse numeric values, handling NA"""
...
def extract_simple_image_features(image_path):
    ...
    try:
        ...
        # Color statistics per channel (R, G, B)
...
def get_property_image_features(reference):
    """Get aggregated image features for a property"""
...
def load_and_process_data():
    ...
    print(f"✓ Validžių kainų: {valid_prices:,} ({valid_prices/len(df)*100:.1f}%)")
    ...
    # Parse features into binary columns
    print("\nApdorojami požymiai...")
    all_features = set()
    ...
    print(f"✓ Rasta unikalių požymių: {len(all_features)}")
...
def extract_all_image_features(references):
    """Extract image features for all properties in parallel"""
    print("\n" + "=" * 60)
    print("2. VAIZDŲ APDOROJIMAS")
    print("=" * 60)
    
    if not os.path.exists(IMAGES_DIR):
        print(f"Įspėjimas: Nerastas vaizdų katalogas {IMAGES_DIR}")
    ...
    with ThreadPoolExecutor(max_workers=NUM_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            ...
            completed += 1
            if completed % 1000 == 0 or completed == len(references):
    ...
    elapsed = time.time() - start_time
    print(f"✓ Vaizdų apdorojimas baigtas per {elapsed:.1f}s")
...
def prepare_training_data(df, all_features):
    ...
    print("\n" + "=" * 60)
    print("3. POŽYMIŲ PARUOŠIMAS")
    print("=" * 60)
    
    # Filter rows with valid prices
    df_valid = df[df['price_numeric'].notna()].copy()
    print(f"Validžių įrašų su kainomis: {len(df_valid):,}")
...
def train_model(X, y):
    ...
    # Train Gradient Boosting model (better for larger datasets)
